...
export default function FlashcardDeck({ flashcards = [] }) {
  ...
  // Preprocess flashcards to ensure they are in the correct format
  useEffect(() => {
    if (typeof flashcards === "string") {
      // If flashcards is a string, split it into an array of objects
      const parsedFlashcards = flashcards
        .split(";") // Split by semicolon to separate each flashcard
        .map((item) => {
          const [question, answer] = item.split(":").map((str) => str.trim()); // Split by colon and trim
          return { Question: question || "No Question", Answer: answer || "No Answer" };
        });
      setProcessedFlashcards(parsedFlashcards);
    } else if (Array.isArray(flashcards)) {
      // If flashcards is an array, ensure each item is an object with Question and Answer
      const parsedFlashcards = flashcards.map((item) => {
        if (typeof item === "string") {
          const [question, answer] = item.split(":").map((str) => str.trim());
          return { Question: question || "No Question", Answer: answer || "No Answer" };
        }
        return item; // Assume it's already an object
      });
      setProcessedFlashcards(parsedFlashcards);
    } else {
      // If flashcards is not a string or array, set it to an empty array
  ...
  const handleNext = () => {
    ...
    setCurrentCardIndex((prevIndex) =>
      prevIndex === processedFlashcards.length - 1 ? 0 : prevIndex + 1
    );
  };
    ...
    <div className="flex flex-col items-center w-full max-w-3xl mx-auto p-4">
      ...
      <div className="w-full mb-6">
        <div className="flex justify-between text-sm text-gray-600 mb-2">
          ...
          <span>
            {Math.round(((currentCardIndex + 1) / processedFlashcards.length) * 100)}% Complete
          </span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2.5">
          <div
            ...
            style={{
              width: `${((currentCardIndex + 1) / processedFlashcards.length) * 100}%`,
            }}
          ></div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <div className="bg-blue-50 text-blue-600 font-medium text-sm px-3 py-1 rounded-full w-fit mb-4">
              Question
            </div>
          ...
          >
            <div className="bg-blue-100 text-blue-700 font-medium text-sm px-3 py-1 rounded-full w-fit mb-4">
              Answer
            </div>
